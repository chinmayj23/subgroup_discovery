...
# src/longitudinal_wbd.py
import temporian as tp

def build_country_snapshot_temporian(
    df, 
    id_col, 
    date_col, 
    target_col, 
    ...
    """
    Super-fast feature engineering using Temporian.
    Strictly separates 'Labeling' features (trends) from 'Explanation' features (lags).
    """
    # 1. Convert to Temporian EventSet
    #    This is instant (zero-copy if data types align)
    ...
    # 2. Define the Target Node
    target_node = evset[target_col]

    # --- PART A: Labeling Features (The "Wrong" Questions) ---
    # These are used ONLY to find/cluster the subgroups.
    # We calculate trends/means here so you can label based on them if you want.
    # Note: Temporian handles windows correctly respecting time.
    target_trend = target_node.simple_moving_average(window_length=5*365) # Approx 5 years
    target_stats = tp.glue(
        target_node.prefix("target_raw_"),
        target_trend.prefix(f"{target_col}_roll_mean_")
    )

    # --- PART B: Explanation Features (The "Right" Questions) ---
    # We take ALL numeric columns (except target) and create pure LAGS.
    # This answers: "What was the value 1 year ago? 3 years ago?"
    # It strictly strictly prevents "peeking" at the current or future window.
    
    # Select all numeric features
    feature_nodes = evset.drop(target_col)
    
    # Create the Lags (1 year, 3 years, 5 years)
    # We assume '1 year' approx 365 days for WBD data
    # Note: Temporian handles missing dates automatically (it won't grab a wrong row).
    lags_1y = feature_nodes.lag(duration=365).prefix("lag1y_")
    lags_3y = feature_nodes.lag(duration=365*3).prefix("lag3y_")
    lags_5y = feature_nodes.lag(duration=365*5).prefix("lag5y_")
    
    # Create "Deltas" (1-step change) which are valid "Right Questions"
    # Delta = (Value @ t) - (Value @ t-1y)
    # This represents "Recent Change" without averaging.
    delta_1y = (feature_nodes - lags_1y).prefix("delta1y_")

    # Glue "Explanation" features together
    explanation_features = tp.glue(
        feature_nodes.prefix("latest_"), # The value exactly at snapshot time
        lags_1y,
        lags_3y,
        lags_5y,
        delta_1y
    ...
    # We glue the Target stats and the Explanation features
    final_node = tp.glue(target_stats, explanation_features)

    # 4. Filter for valid history
    # We only want rows where we actually HAVE a 3-year history
    # Temporian's .isnan() check is very fast.
    has_history = ~lags_3y[lags_3y.feature_names[0]].isnan()
    final_node = final_node.filter(has_history)

    # 5. Export to Pandas (Snapshotting)
    # If you only want the LAST row per country (Snapshot mode):
    # We take the latest timestamp per index.
    # (In standard pipelines we might keep all, but your code expects snapshots)
    
    # For now, export all valid rows, then deduplicate in Pandas (fastest path)
    final_df = tp.to_pandas(final_node)
    
    # Sort and take last per country to get the "Snapshot"
    final_df = final_df.sort_values([id_col, "timestamp"]).groupby(id_col).tail(1)
...
def run_wbd_longitudinal_pipeline(
    ...
    print("Generating features with Temporian...")
    snapshot = build_country_snapshot_temporian(
        df,
        id_col=id_col,
        date_col=date_col,
    ...
    # The 'snapshot' now contains columns like:
    # - target_raw_life_expectancy
    # - latest_gdp
    # - lag3y_gdp
    # - delta1y_gdp
    actual_target_col = f"target_raw_{target_col}"
